...
c***********************************************************************
      ...
      logical lsolva,lfree,lfrmas,lghost,redirect,lpimd,lmsite
...
c     maximum number of intramolecular void interactions
...
c     maximum number of void interactions per node
      
      mxvoid=max(1,(mxvoid-1)/mxnode+1)
...
c     maximum size for working arrays for bonds, voids, angles, 
c     dihedrals, inversion potentials, tethers and core-shell units
      
      msbad=max(mxbond,mxvoid,mxangl,mxdihd,mxinv,msteth,mxshl)
...
c***********************************************************************
      ...
      integer numbonds,numvoids,ivoids,itpmet,iii,ngrid
...
c     Method Development and Materials Simulation Laboratory
              ...
              elseif(findstring('voids',record,idum))then
                ...
                numvoids=intstr(record,lenrec,idum)
                mxtvod=mxtvod+numvoids
                mxvoid=mxvoid+nummols*numvoids
                
                do ivoids=1,numvoids
...
c *******************************************************************      
      ...
      if(mxtvod.gt.0)mxexcl=max(mxexcl,6)
      if(mxtang.gt.0)mxexcl=max(mxexcl,22)
      if(mxtdih.gt.0)mxexcl=max(mxexcl,56)
      if(mxtinv.gt.0)mxexcl=max(mxexcl,56)
...
c     open the simulation input file
      ...
      do while(peek)
        ...
        if(record(1).ne.'#')then
          ...
          elseif(findstring('qtip4pf',record,idum))then
